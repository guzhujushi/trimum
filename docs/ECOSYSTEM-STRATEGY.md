...
> 安装接线 / 能力交集）与 §7.6（卸载与注销）；官网服务端与目录托管仍未开工。
...
**已知缺口**：官网服务端与目录托管（`DEFAULT_INDEX_URL` 仍是占位）/
多用户边界（§7.2）/ 目录索引的生成入口（`trm pkg index` 待做，见 `TODO.md` E5 第三片步骤 2）/
`--show` 只读 install 子集的剧本自动触发（与 E5 无关的穿插项）。

（`trm install --remove` 已于 §7.6 落地，从缺口里划掉。）
