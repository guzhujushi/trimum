...
> 本文是**设计层**产物：先定「做什么、不做什么、红线在哪、分几步走」。
> **2026-09-21 更新**：§8 的主要待裁决已由用户拍定（见 §8 的「已裁决」块）——
> 沙箱**提到本片之前**（成为前置片 `docs/SANDBOX-PLAN.md`，S1 已就绪）；首发**默认出差异 + 只读验证，写盘要确认**，`--yes` 才自动落盘；
> 入口定为 **`trm exec --code`**（不是独立命令 `trm code`）；`skill.yaml` 与 `SKILL.md` **不合并**。
...
- `trm exec --code "<任务>"`（**裁决：挂在既有 `exec` 下做子模式，不新开独立命令**；命令面仍是 78，只加 flag/子模式）：
  `--dry-run`（只出计划与差异）、`--skill <名>`、`--timeout`、`--yes`；退出码口径与 `trm workflow run` 一致（做了就是 0，被拒/失败就是非 0）。
  > 与既有 `trm exec "<命令>"` 的区别要写清楚：**不带 `--code` 是「跑一条命令」，带 `--code` 是「开一个编码会话」**（多轮、有差异预览与回滚）。
...
| # | 问题 | 裁决 | 影响 |
|---|---|---|---|
| 1 | 沙箱是否提到 E7 之前 | **是** | 先做 `docs/SANDBOX-PLAN.md`（S1 已就绪），E7 顺延 |
| 2 | 首发是否允许自动改盘 + 自动跑测试 | **默认只出差异 + 跑只读验证**；写盘要确认，`--yes` 才自动落盘 | 步骤 1/2 的默认行为 |
| 3 | 命令入口 | **`trm exec --code`**（挂在既有 `exec` 下），不新开 `trm code` | 步骤 5 |
| 5 | 两套技能是否合并 | **不合并**，明确分工（`skill.yaml` = 能力，`SKILL.md` = 说明） | 步骤 3 |

### 8.2 仍未决

1. **沙箱与「安全优先」的正面冲突**：编码智能体会大量写文件、跑测试，而 Landlock / Seccomp 至今未做。
   问题：是否把沙箱提到 E7 之前 —— **已裁决：是**（见 §8.1）。
2. **首发能力边界**（**已裁决**，见 §8.1）：默认「出差异 + 跑只读验证」，写盘要确认；`--yes` 才自动落盘。
3. **命令入口**（**已裁决**）：`trm exec --code`。
4. **工具调用方式**：继续「让模型吐单步 JSON」还是切**原生工具调用**（模型侧支持函数调用）？
   建议本片先不切（改动面小、可离线降级），但把工具 schema 输出预留好（`trm commands --json` 已经是雏形）。
5. **两套技能是否合并**（**已裁决：不合并**，见 §8.1）。
...
> **下一步**：§8.1 的四条已定，**先做沙箱前置片**（`docs/SANDBOX-PLAN.md` 的 S1 → S4），
> 再回来按 §7 的步骤 1→5 推进（入口用 `trm exec --code`）。仍有两项未决（原生工具调用 / 会话记录位置），不阻塞开工。
