"""Generate remaining theme colors.conf files for Trimum Ubuntu."""
import os
...
template = """# {name} — Trimum Ubuntu Theme
...
for name, variant, bg, surface, overlay, subtext, text, accent, red, green, yellow, blue, pink in themes:
    theme_dir = os.path.join(themes_dir, name)
    os.makedirs(os.path.join(theme_dir, "backgrounds"), exist_ok=True)
    
    content = template.format(
        name=name, variant=variant, bg=bg, surface=surface, overlay=overlay,
        subtext=subtext, text=text, accent=accent, red=red, green=green,
        yellow=yellow, blue=blue, pink=pink
    )
...
print(f"Done. Generated {len(themes)} themes.")
